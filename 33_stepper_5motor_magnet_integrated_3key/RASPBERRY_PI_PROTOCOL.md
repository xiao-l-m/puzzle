...
- KEY3/PB19：向树莓派发送 `KEY seq 3`，请求第三问未知白片矩形方案。
- PB21：急停；GPIO下降沿会先立即关闭PA14电磁铁，再由主循环停止1~5号电机。

KEY1/KEY2/KEY3不会直接移动。树莓派没有返回完整、合法且坐标可达的计划时，所有电机保持不动。模式1/2要求4片，模式3允许1~4片。
...
XY可达范围固定为：`X=5.0..205.0mm`、`Y=62.0..287.0mm`。
...
当前固件握手标识为 `READY GANTRY_V3_XY_ROTATE ...`。`MOVE_PLACE_ROTATE` 状态会先后发送5号轴和XY命令使其并行执行，共用截止时间取两项保守完成时间的较大值，避免XY尚未到位就提前下降Z轴。
