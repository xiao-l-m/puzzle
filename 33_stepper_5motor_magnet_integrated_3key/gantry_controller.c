...
typedef enum {
    ...
    STATE_WAIT_PROBE_UP,
    STATE_WAIT_PROBE_XY,
    STATE_WAIT_PROBE_DOWN,
    STATE_WAIT_PROBE_HOME_UP,
    STATE_WAIT_PROBE_HOME_XY,
    STATE_ESTOP,
    STATE_FAULT
} ControllerState;
...
static const char *state_name(ControllerState state)
{
    switch (state) {
        ...
        case STATE_WAIT_PROBE_UP: return "PROBE_Z_UP";
        case STATE_WAIT_PROBE_XY: return "PROBE_XY";
        case STATE_WAIT_PROBE_DOWN: return "PROBE_Z_DOWN";
        case STATE_WAIT_PROBE_HOME_UP: return "PROBE_HOME_Z_UP";
        case STATE_WAIT_PROBE_HOME_XY: return "PROBE_HOME_XY";
...
static MotionStart start_z_move(bool upward, uint32_t nowMs)
{
    ...
    duration = linear_profile_duration_ms(GANTRY_Z_TRAVEL_X10,
                                          GANTRY_Z_SPEED_RPM,
...
void GantryController_Init(uint32_t nowMs)
{
    ...
    (void)RpiLink_SendFormat(
        "READY GANTRY_V4_WEB_PROBE HOME %ld %ld RANGE %ld %ld %ld %ld",
...
static void begin_probe_down(uint32_t nowMs)
{
    send_state("PROBE_Z_DOWN");
    g_controller.zSafe = false;
    if (start_z_move(false, nowMs) == MOTION_FAILED) {
        enter_fault("PROBE_Z_DOWN_TX");
    } else {
        g_controller.state = STATE_WAIT_PROBE_DOWN;
    }
}

static void begin_probe_xy(uint32_t nowMs)
{
    MotionStart result;

    send_state("PROBE_XY");
    result = start_xy_move(g_controller.pendingX10,
                           g_controller.pendingY10, nowMs);
    if (result == MOTION_FAILED) {
        enter_fault("PROBE_XY_TX");
    } else if (result == MOTION_SKIPPED) {
        g_controller.currentX10 = g_controller.pendingX10;
        g_controller.currentY10 = g_controller.pendingY10;
        begin_probe_down(nowMs);
    } else {
        g_controller.state = STATE_WAIT_PROBE_XY;
    }
}

static void begin_probe_home_xy(uint32_t nowMs)
{
    MotionStart result;

    send_state("PROBE_HOME_XY");
    result = start_xy_move(GANTRY_HOME_X_X10, GANTRY_HOME_Y_X10, nowMs);
    if (result == MOTION_FAILED) {
        enter_fault("PROBE_HOME_XY_TX");
    } else if (result == MOTION_SKIPPED) {
        g_controller.currentX10 = GANTRY_HOME_X_X10;
        g_controller.currentY10 = GANTRY_HOME_Y_X10;
        finish_request();
    } else {
        g_controller.state = STATE_WAIT_PROBE_HOME_XY;
    }
}

static bool probe_preconditions_valid(uint32_t request)
{
    if (g_controller.state != STATE_IDLE) {
        (void)RpiLink_SendFormat("ERR %lu BUSY", (unsigned long)request);
        return false;
    ...
    if (!g_controller.positionKnown || MOS_Switch_IsOn() ||
        (g_controller.rotationMdeg != 0)) {
        (void)RpiLink_SendFormat("ERR %lu PROBE_NOT_SAFE",
                                 (unsigned long)request);
        return false;
    }
    return true;
...
static void process_probe(char *tokens[TOKEN_COUNT_MAX], uint8_t count,
                          uint32_t nowMs)
{
    uint32_t request;
    int32_t x10;
    ...
    if ((count != 4U) || !parse_u32(tokens[1], &request) ||
        !parse_i32(tokens[2], &x10) || !parse_i32(tokens[3], &y10) ||
        (request == 0U)) {
        (void)RpiLink_SendLine("ERR 0 BAD_PROBE");
        return;
    }
    if (!probe_preconditions_valid(request)) {
        return;
    }
    if (!coordinate_is_reachable(x10, y10)) {
        (void)RpiLink_SendFormat("ERR %lu RANGE", (unsigned long)request);
    ...
    g_controller.requestId = request;
    g_controller.jobIndex = 0U;
    g_controller.pendingX10 = x10;
    g_controller.pendingY10 = y10;
    (void)RpiLink_SendFormat("ACK %lu PROBE", (unsigned long)request);
    if (!g_controller.zSafe) {
        send_state("PROBE_Z_UP");
        if (start_z_move(true, nowMs) == MOTION_FAILED) {
            enter_fault("PROBE_Z_UP_TX");
        } else {
            g_controller.state = STATE_WAIT_PROBE_UP;
        }
    } else {
        begin_probe_xy(nowMs);
    }
}

static void process_probe_home(char *tokens[TOKEN_COUNT_MAX], uint8_t count,
                               uint32_t nowMs)
{
    uint32_t request;
    ...
    if ((count != 2U) || !parse_u32(tokens[1], &request) ||
        (request == 0U)) {
        (void)RpiLink_SendLine("ERR 0 BAD_PROBEHOME");
        return;
    }
    if (!probe_preconditions_valid(request)) {
        return;
    ...
    g_controller.pendingX10 = GANTRY_HOME_X_X10;
    g_controller.pendingY10 = GANTRY_HOME_Y_X10;
    (void)RpiLink_SendFormat("ACK %lu PROBEHOME", (unsigned long)request);
    if (!g_controller.zSafe) {
        send_state("PROBE_HOME_Z_UP");
        if (start_z_move(true, nowMs) == MOTION_FAILED) {
            enter_fault("PROBE_HOME_Z_UP_TX");
        } else {
            g_controller.state = STATE_WAIT_PROBE_HOME_UP;
        }
    } else {
        begin_probe_home_xy(nowMs);
    }
}
...
void GantryController_ProcessLine(char *line, uint32_t nowMs)
{
    ...
    if (strcmp(tokens[0], "STOP") == 0) {
        ...
        process_probe(tokens, count, nowMs);
    } else if (strcmp(tokens[0], "PROBEHOME") == 0) {
        process_probe_home(tokens, count, nowMs);
        ...
        (void)RpiLink_SendLine(
            "COMMANDS PING STATUS STOP PLAN ITEM COMMIT REJECT MOVEA MOVER PROBE PROBEHOME PICKPLACE");
    } else {
