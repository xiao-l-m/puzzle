...
#define TASK_KEY_COUNT       3U
#define COMPLETE_LED_MS      5000U
#define REJECT_LED_MS        4000U
#define COMPLETE_LED_HALF_PERIOD_MS 500U
#define REJECT_LED_HALF_PERIOD_MS   200U
...
static void task_keys_poll(uint32_t nowMs)
{
    ...
    for (index = 0U; index < TASK_KEY_COUNT; index++) {
        ...
        if ((state->samples == KEY_DEBOUNCE_SAMPLES) &&
            ...
            state->stable = pressed;
            if (pressed != 0U) {
                /* KEY1/KEY2/KEY3 start integrated modes 1/2/3. */
...
int main(void)
{
    ...
    while (1) {
        ...
        if (GantryController_IsEmergencyStopped()) {
            ...
            if (((nowMs / REJECT_LED_HALF_PERIOD_MS) & 1U) != 0U) {
                DL_GPIO_setPins(LED1_PORT, LED1_PIN_22_PIN);
            } else {
                DL_GPIO_clearPins(LED1_PORT, LED1_PIN_22_PIN);
            }
        } else if ((int32_t)(g_completeLedUntilMs - nowMs) > 0) {
            if (((nowMs / COMPLETE_LED_HALF_PERIOD_MS) & 1U) != 0U) {
                DL_GPIO_setPins(LED1_PORT, LED1_PIN_22_PIN);
            } else {
                DL_GPIO_clearPins(LED1_PORT, LED1_PIN_22_PIN);
            }
