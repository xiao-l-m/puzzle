# E题三模式：搬运、固定拼图与未知碎片重建

本工程包含三个网页任务模式：

- 按键1：识别下半区的4片镀锌铁片，全部搬到分界线上方；优先保持方向，空间不足时自动旋转90°后紧凑排布。
- 按键2：识别并匹配题图2的4片，在上半区靠近分界线拼成 `100 mm × 60 mm` 矩形。
- 模式3：识别下半区1～4片未知白色碎片，自动重建规定尺寸矩形并搬到分界线上半区；无需固定模板。

三个模式都在运动前冻结完整视觉和动作计划。搬走第一片后，即使实时画面中的碎片数量变化，也不会修改本次计划。
...
- X向右、Y向下，单位mm
- 固定初始磁铁中心：A4 `(5, 62) mm`
- 当前实测可达质心：`X=5..205 mm`，`Y=62..287 mm`
- 1号电机正转向左，所以A4 `+X` 对应1号电机反转
- 2、3号电机正转向下，与A4 `+Y` 一致
- 4号电机正转为Z向上；每次拾取/放置下降8mm、再上升8mm
- 5号电机负责铁片旋转；`--rotation-sign` 必须在 `+90°` 实测后确定

Y总行程只有225mm，不能覆盖297mm长的整张A4。本方案因此固定为“下半区取件、上半区靠分界线放置”：松散铁片质心约在 `Y=150..287 mm`，目标矩形约在 `Y=divider-65..divider-5 mm`，避开顶部 `Y<62 mm` 的机械盲区。网页仍会逐点检查，越界时整份计划拒绝。
...
模式3网页按钮是真正的一键启动：点击后自动等待 A4 标定和1～4片白色碎片稳定，完成未知矩形求解、质心映射、旋转角和可达性检查后再下发计划，不需要二次确认。当前实物方向为下半区取件、上半区靠近分界线放置。
...
每片严格执行：安全高度到拾取XY → Z下降8mm → 磁铁开启 → Z抬升 → XY移向放置点并与5轴旋转同时进行 → 两者都结束并停稳 → Z下降 → 磁铁关闭 → Z抬升 → 5轴回零。XY和旋转只允许在Z安全高度运动。
...
- `POST /api/task?mode=3`：一键等待识别、求解并执行未知白片矩形重建
- `POST /api/config`：更新视觉阈值及模式3放置X/Y补偿
- `POST /api/stop`：急停

## 第二大问第（1）小问

下半区1～4片白色碎片自动重建并搬到分界线上半区的模式3说明见 [MODE3_QUESTION2_1.md](MODE3_QUESTION2_1.md)。目标矩形默认沿分界线水平居中，网页可对最终放置点增加小范围X/Y补偿。
